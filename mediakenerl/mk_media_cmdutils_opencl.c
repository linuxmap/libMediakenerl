...
static int64_t mk_run_opencl_bench(AVOpenCLExternalEnv *ext_opencl_env)
{
...
static int mk_compare_ocl_device_desc(const void *a, const void *b)
{
...
int mk_opt_opencl_bench(mk_task_ctx_t* task,void *optctx, const char *opt, const char *arg)
{
    ...
    for (i = 0; i < device_list->platform_num; i++) {
        for (j = 0; j < device_list->platform_node[i]->device_num; j++) {
            ...
            score = av_opencl_benchmark(device_node, platform, mk_run_opencl_bench);
            if (score > 0) {
    ...
    qsort(devices, count, sizeof(OpenCLDeviceBenchmark), mk_compare_ocl_device_desc);
