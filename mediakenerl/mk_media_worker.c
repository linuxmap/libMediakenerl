...
static void mk_sub2video_update(mk_input_stream_t *ist, AVSubtitle *sub)
{
    AVFrame *frame = ist->sub2video.frame;
    int8_t *dst;
    int     dst_linesize;
...
static void mk_write_packet(mk_task_ctx_t* task,mk_output_file_t*of, AVPacket *pkt,mk_output_stream_t *ost)
{
    ...
    if (ost->nb_bitstream_filters) {
        ...
        while (idx) {
            ...
            if (idx < ost->nb_bitstream_filters) {
                ...
                mk_write_packet(task,of, pkt, ost);
        }
    } else
        mk_write_packet(task,of, pkt, ost);
...
static int mk_decode_audio(mk_task_ctx_t* task,mk_input_stream_t *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (resample_changed) {
        ...
        if (!mk_guess_input_channel_layout(ist)) {
        ...
        for (i = 0; i < task->nb_filtergraphs; i++)
            if (mk_ist_in_filtergraph(task->filtergraphs[i], ist)) {
                mk_filter_graph_t *fg = task->filtergraphs[i];
                if (mk_configure_filtergraph(task,fg) < 0) {
...
static int mk_decode_video(mk_task_ctx_t* task,mk_input_stream_t *ist, AVPacket *pkt, int *got_output, int eof)
{
    ...
    if (resample_changed) {
        ...
        for (i = 0; i < task->nb_filtergraphs; i++) {
            if (mk_ist_in_filtergraph(task->filtergraphs[i], ist) && ist->reinit_filters &&
                mk_configure_filtergraph(task,task->filtergraphs[i]) < 0) {
                av_log(NULL, AV_LOG_FATAL, "Error reinitializing filters!\n");
                task->running = 0;
                goto fail;
                //exit_program(task,1);
...
static int mk_process_input_packet(mk_task_ctx_t* task,mk_input_stream_t *ist, const AVPacket *pkt,int no_eof)
{
    ...
    while (ist->decoding_needed) {
        ...
        case AVMEDIA_TYPE_SUBTITLE:
            if (repeating)
                break;
            ret = mk_transcode_subtitles(task,ist, &avpkt, &got_output);
            if (!pkt && ret >= 0)
                ret = AVERROR_EOF;
            break;
        default:
...
static int mk_init_output_stream(mk_task_ctx_t* task,mk_output_stream_t *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if ((ret = avcodec_open2(ost->enc_ctx, codec, &ost->encoder_opts)) < 0) {
            if (ret == AVERROR_EXPERIMENTAL){
                //abort_codec_experimental(task,codec, 1);
                return -1;
            }
            snprintf(error, error_len,
        ...
        mk_assert_avoptions(task,ost->encoder_opts);
...
static int mk_transcode_init(mk_task_ctx_t* task)
{
    ...
    for (i = 0; i < task->nb_output_streams; i++) {
        ...
        ist = mk_get_input_stream(task,ost);
        ...
        if (!ost->stream_copy) {
...
#if CONFIG_CUVID
            if (mk_cuvid_transcode_init(task,ost)){
                task->running = 0;
                ret = -1;
                goto dump_format;
            }
            ...
            if ((enc_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
                 ...
                 mk_filtergraph_is_simple(ost->filter->graph)) {
                    mk_filter_graph_t*fg = ost->filter->graph;
                    if (mk_configure_filtergraph(task,fg)) {
