...
static int mk_vaapi_get_buffer(AVCodecContext *avctx, AVFrame *frame, int flags)
{
...
static int mk_vaapi_retrieve_data(AVCodecContext *avctx, AVFrame *input)
{
    mk_input_stream_t *ist = avctx->opaque;
    VAAPIDecoderContext *ctx = ist->hwaccel_ctx;
    AVFrame *output = 0;
...
static int mk_vaapi_build_decoder_config(VAAPIDecoderContext *ctx,
...
static void mk_vaapi_decode_uninit(AVCodecContext *avctx)
{
    mk_input_stream_t *ist = avctx->opaque;
    VAAPIDecoderContext *ctx = ist->hwaccel_ctx;
...
int mk_vaapi_decode_init(AVCodecContext *avctx)
{
    ...
    if (!hw_device_ctx) {
        err = mk_vaapi_device_init(ist->hwaccel_device);
        if (err < 0)
            return err;
    }
    ...
    ist->hwaccel_ctx           = ctx;
    ist->hwaccel_uninit        = mk_vaapi_decode_uninit;
    ist->hwaccel_get_buffer    = mk_vaapi_get_buffer;
    ist->hwaccel_retrieve_data = mk_vaapi_retrieve_data;
...
av_cold int mk_vaapi_device_init(const char *device)
{
    int err;
