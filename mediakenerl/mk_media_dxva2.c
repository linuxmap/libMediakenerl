...
static void mk_dxva2_uninit(AVCodecContext *s)
{
...
static int mk_dxva2_get_buffer(AVCodecContext *s, AVFrame *frame, int flags)
{
    InputStream  *ist = s->opaque;
    DXVA2Context *ctx = ist->hwaccel_ctx;
...
static int mk_dxva2_retrieve_data(AVCodecContext *s, AVFrame *frame)
{
    InputStream        *ist = s->opaque;
    DXVA2Context       *ctx = ist->hwaccel_ctx;
    int                ret;
...
static int mk_dxva2_alloc(AVCodecContext *s)
{
    InputStream  *ist = s->opaque;
    ...
    ist->hwaccel_ctx           = ctx;
    ist->hwaccel_uninit        = mk_dxva2_uninit;
    ist->hwaccel_get_buffer    = mk_dxva2_get_buffer;
    ist->hwaccel_retrieve_data = mk_dxva2_retrieve_data;
...
static int mk_dxva2_get_decoder_configuration(AVCodecContext *s, const GUID *device_guid,
...
static int mk_dxva2_create_decoder(AVCodecContext *s)
{
    InputStream  *ist = s->opaque;
    int loglevel = (ist->hwaccel_id == HWACCEL_AUTO) ? AV_LOG_VERBOSE : AV_LOG_ERROR;
    DXVA2Context *ctx = ist->hwaccel_ctx;
    ...
    ret = mk_dxva2_get_decoder_configuration(s, &device_guid, &desc, &config);
    if (ret < 0) {
        goto fail;
    }
