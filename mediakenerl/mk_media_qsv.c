...
static void mk_buffer_release(void *opaque, uint8_t *data)
{
    *(uint8_t*)opaque = 0;
}
...
static int mk_init_opaque_surf(QSVContext *qsv)
{
...
static mfxIMPL mk_choose_implementation(const InputStream *ist)
{
    static const struct {
        const char *name;
        mfxIMPL     impl;
...
int mk_qsv_transcode_init(mk_task_ctx_t* task,mk_output_stream_t *ost)
{
    ...
    for (i = 0; i < task->nb_output_streams; i++)
        if (task->output_streams[i] != ost &&
            task->output_streams[i]->source_index == ost->source_index)
            return 0;
    ...
    impl = mk_choose_implementation(ist);
